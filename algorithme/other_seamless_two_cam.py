...
def intensity_all_views(views, channel_id, vertex_in_mesh, Vjyxc_to_103, N):
    """
    ...
    for view_id in tqdm(range(len(views)), desc="Calcul de intensity_all_views"):
        rot, t = get_image_data(view_id%N)
        cam = "l" if view_id < N else "r"
        for (vertex_id, vertex_mesh) in vertex_in_mesh[view_id]:
            pixel = back_projeter(vertex_mesh, rot, t, cam)[0]
            x, y = int(pixel[0]), int(pixel[1])
            all_views_intensities[(vertex_id, view_id)] = Vjyxc_to_103[view_id, y, x, channel_id]
...
if __name__ == "__main__":
    ...
    h, w = 2000, 3000
    Vjyxc_to_103 = np.zeros((2 * N, h, w, 3), dtype=int)

    for j in range(1, N + 1):
        img_r = cv2.cvtColor(cv2.imread(f"downsampled/scene_r_{j:04d}.jpeg"), cv2.COLOR_BGR2RGB)
        img_l = cv2.cvtColor(cv2.imread(f"downsampled/scene_l_{j:04d}.jpeg"), cv2.COLOR_BGR2RGB)
        
        Vjyxc_to_103[N + j - 1] = img_r
        Vjyxc_to_103[j - 1] = img_l

    print(f"Vjyxc_to_103 chargé. Shape : {Vjyxc_to_103.shape}")


    # Création du dictionnaire de vues
    views = {i: Vjyxc_to_103[i] for i in range(2*N)}

    # Chargement du tensor M_final
    M_final = np.load("tensors/M_final_depth9.npy")

    # --- Chargement et prép + 1)
    mesh = trimesh.load("ply/clean_mesh_cailloux_luca_depth9.ply")
    ...
    red_intensity_all_views = intensity_all_views(views, 0, vertex_in_mesh, Vjyxc_to_103, N)
    blue_intensity_all_views = intensity_all_views(views, 2, vertex_in_mesh, Vjyxc_to_103, N)
    green_intensity_all_views = intensity_all_views(views, 1, vertex_in_mesh, Vjyxc_to_103, N)


    # np.save("tensors/red_intensity_all_views.npy",red_intensity_all_views,allow_pickle=True)
    # np.save("tensors/blue_intensity_all_views.npy", blue_intensity_all_views,allow_pickle=True)
    # np.save("tensors/green_intensity_all_views.npy", green_intensity_all_views,allow_pickle=True)
    ...
    for face_id in tqdm(range(len(indices)), desc="Traitement des faces"):
        ...
        for x, y in zip(cc, rr):
            ...
            try : 
                pixel = back_projeter(pt_3d, rot, t, cam)[0]
            except :
                pixel = None
            ...
            final_colors[(y, x)] = np.clip(intensity + color, 0, 255) # intensity  + color,
