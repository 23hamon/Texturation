...
def clean_mesh(old_mesh, rot_images, t_images, cos_theta_max=0):
    ...
    **Input**:
        ...
        - rot_images (np.ndarray ou liste): rotations des cameras
        - t_images (np.ndarray ou liste): translations des cameras
        - cos_theta_max (float): seuil de visibilite
    **Output** :
        - new_mesh : o3d.geometry.TriangleMesh: un nouveau mesh avec seulement les faces visibles
        - new_Mpj (np.ndarray): la matrice de visibilite correspondant aux faces du nouveau mesh
    """
    old_Mpj = build_Mpj(old_mesh, rot_images, t_images, "l", cos_theta_max)

    visible_faces_mask = np.any(old_Mpj, axis=1)  # shape: (nb_faces,)
    new_Mpj = old_Mpj[visible_faces_mask]
...
if __name__ == "__main__" :
    ...
    new_mesh, new_Mpj = clean_mesh(mesh, rot_images, t_images)
    ...
    from visible_faces_from_view import reconstruct_visible_mesh
    visible_mesh = reconstruct_visible_mesh(new_mesh, new_Mpj[:, 45])
