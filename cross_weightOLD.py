...
def _cross_weight_seam(Vjyxc,                   # tenseur images
                       t_linspace, X1, X2,      # echantillonnage de l'arete : t ~= [0,1]
                       j, k,                    # vues j et k
                       rotj, tj, rotk, tk,
    ...
    """
    y = np.array([_weight_point_on_seam(Vjyxc, (X2*t+(1-t)*X1), j, k, rotj, tj, rotk, tk, cam) for t in t_linspace])
    integr = sqrt_newton(((X2-X1)**2).sum()) * trapezoidal_integration(t_linspace, y)
    return integr
...
def Wpqjk(N, Vjyxc, cam,              # donnees sur les images
            ...
            p, q, j, k,
            t_linspace=np.linspace(0, 1, 100)    # intervalle [0,1] pour integrer
            ):
    """
    ...
    if Mpj[p, j] and Mpj[q, j] and  Mpj[p, k] and Mpj[q, k] : # On impose que les deux faces soient visibles sur les deux vues
        ...
        if p < q : # si p et q sont bien ordonnes
            if (j, k) in Wpqjk_dict[(p, q)] :
                ...
                integr = _cross_weight_seam(Vjyxc, t_linspace, X1, X2, j, k, rot_j, t_j, rot_k, t_k, cam)
...
if __name__ == "__main__" :
    ...
    time_flag = time.time()
    vues_aleatoires = [(random.randint(0, 51), random.randint(0, 51)) for tyty in range(N_rand)]
    for idx, cle in enumerate(cles_aleatoires) :
        p, q = cle
        j, k = vues_aleatoires[idx]
        print(f"W_{p},{q},{j},{k} = {Wpqjk(N, Vjyxc, cam, rot_images, t_images, vertices, edges_set, Mpj, p, q, j, k, t_linspace)}")
    new_time_flag = time.time()
    print(f"Temps d'execution : {new_time_flag-time_flag}. Et maintenant :\n\n")
    for idx, cle in enumerate(cles_aleatoires) :
        p, q = cle
        j, k = vues_aleatoires[idx]
        #print(f"W_{p},{q},{j},{k} = {Wpqjk(N, Vjyxc, cam, rot_images, t_images, vertices, edges_set, Mpj, p, q, j, k)}")
    print(f"Temps d'execution : {time.time() - new_time_flag}")
    print(sqrt_newton(200000))
    # #### test
    # imid = 38
