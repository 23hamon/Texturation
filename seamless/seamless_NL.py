...
# im = plt.imshow(intensity(2, 0), origin='lower')
# plt.axis('off')
# plt.title('Intensité VERTE Vue 2')
# plt.colorbar(im, label="Intensité")
# plt.show()
print(intensity(0, 0)[0,0])

def get_intensity_vertice(view_id, vertex_id, color_channel):
    image_size = views[view_id].shape[:2]
    uvs = uvs[mesh.faces[vertex_id]]
    uvs = uvs * ima
    f = intensity(view_id, color_channel)
    print(x, y)
    return f[y, x]
    

print("Intensité du vertex 0 dans la vue 0 (canal rouge) :", get_intensity_vertice(0, 0, 0))
