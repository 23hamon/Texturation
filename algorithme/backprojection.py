...
# def cost_fn(Y, X, R_cam, t_cam, cam=0):
#     if cam==0 :
#         r0, rd = FASTr0_rd(Y, R_cam, t_cam, "l")
#         return np.linalg.norm(distance_X_to_D_r0_rd(X, r0, rd))
#     else :
#         r0, rd = FASTr0_rd(Y, R_cam, t_cam, "r")
#         return np.linalg.norm(distance_X_to_D_r0_rd(X, r0, rd))

# @njit(error_model="numpy", cache=True)
# def approximate_gradient(cost_fn, p, X, R_cam, t_cam, cam, epsilon=1e-6):
...
#         p_neg = p.copy()
#         p_neg[i] -= epsilon
#         f_pos = cost_fn(p_pos, X, R_cam, t_cam, cam)
#         f_neg = cost_fn(p_neg, X, R_cam, t_cam, cam)
#         grad[i] = (f_pos - f_neg) / (2 * epsilon)
#     return grad

# @njit(error_model="numpy", cache=True)
# def gradient_descent(cost_fn, p_init, X, R_cam, t_cam, cam, learning_rate=0.1, max_iter=1000, tol=1e-6):
#     p = p_init.copy()
#     for _ in range(max_iter):
#         grad = approximate_gradient(cost_fn, p, X, R_cam, t_cam, cam)
...
#         Y = gradient_descent(cost_fn, Y0, X, R_cam, t_cam, 0)
#         r0, rd = FASTr0_rd(Y, R_cam, t_cam, cam)
#         return Y, r0, rd
#     else :
#         Y0 = np.array([0.,0.])
#         Y = gradient_descent(cost_fn, Y0, X, R_cam, t_cam, 1)
#         r0, rd = FASTr0_rd(Y, R_cam, t_cam, cam)
#         return Y, r0, rd

# @njit(error_model="numpy", cache=True)
# def back_projeter_numba(X,
#                   R_cam=np.eye(3),      # rotation de la camera
#                   t_cam=np.zeros((3,)), # translation de la camera
#                   cam=0,              # type de camera (gauche ou droite)
#                   ) :
#     """
#     Renvoie Y, la projection inverse sur l'image d'un point X sur le mesh 3D
#     **Input :**
...
#     Y0 = np.array([0.,0.])
#     Y = gradient_descent(cost_fn, Y0, X, R_cam, t_cam, cam)
#     r0, rd = FASTr0_rd(Y, R_cam, t_cam, "r" if cam else "l")
#     return Y, r0, rd
...
#     #Y0 = np.array([1500., 1000.], dtype=np.float64)
#     Y0 = np.array([1500., 1000.], dtype=np.float32)
#     # minimisation
#     res = least_squares(f, Y0, loss="linear", verbose=0, max_nfev=14)
#     #res = least_squares(f, Y0, loss="linear", verbose=2, ftol=1e-10, xtol=1e-10, gtol=1e-10)
#     if max_cost:
#         if res.fun < max_cost :
#             return res.x, r0, rd
#         else : 
