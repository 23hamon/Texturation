...
def _cost_face_cam(rdG, rdD, n_p, rot, t, cam, cost_func, float_inf) :
    """
    Donne le poids d'une face vue depuis une cam donnee
    - rdG, rdD : les vecteurs directeurs des rayons allant a X depuis les cam gauches et droites
...
def build_Wpj(K, N, triangles, vertices, normals, Mpj, rot_images, t_images, cam="l", float_inf=1e20) :
    ...
    """
    all_X = np.zeros((K, 3)) # tableau de chaque face
    for p in range(K) :
        tri = triangles[p]
        X_p = vertices[tri].mean(axis=0)
        all_X[p, :] = X_p

    j_to_rd = np.zeros((N, 2, K, 3), dtype=np.float64)
    print("Retro projection de chaque X_p pour obtenir l'angle avec la camera")
    for j in tqdm(range(N)) :
        rot_j, t_j = rot_images[j], t_images[j]
        all_rdG = np.array([back_projeter(X_p, rot_j, t_j, "l")[2] for X_p in all_X])
        all_rdD = np.array([back_projeter(X_p, rot_j, t_j, "r")[2] for X_p in all_X])
        j_to_rd[j] = np.stack([all_rdG, all_rdD], axis=0)
    ...
    for j in tqdm(range(N)) :
        ...
        for p in (range(K)) :
            if Mpj[p, j] == True :  # si la face est visible
                Wpj[p, j] = _cost_face_cam(all_rdG[p], all_rdD[p], normals[p, :], rot_images[j], t_images[j], cam, _cost_func, float_inf)
    return Wpj
        


if __name__ == "__main__" :
    mesh = o3d.io.read_triangle_mesh("ply/mesh_cailloux_luca_HIGH_CLEAN.ply")
