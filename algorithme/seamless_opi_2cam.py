...
def from_face_to_all_pixel(face_id, view_id, image_shape, indices, uvs, vmapping, mesh, N):
    """
    ...
    for i in range(len(rr)):
        ...
        pixel = back_projeter(point_3d, rot, t, cam)[0]
        if pixel is not None:
            X, Y = int(pixel[0]), int(pixel[1])
            pixels.append((X, Y))
    return pixels

def intensity_all_views(views, channel_id, vertex_in_mesh, Vjyxc, N):
    """
    ...
    for view_id in tqdm(range(2 * N), desc="Calcul de intensity_all_views"):
        rot, t = get_image_data(view_id % N)
        for (vertex_id, vertex_mesh) in vertex_in_mesh[view_id]:
            if view_id < N:
                pixel = back_projeter(vertex_mesh, rot, t, "l")[0]
            else : 
                pixel = back_projeter(vertex_mesh, rot, t, "r")[0]
...
if __name__ == "__main__":
    ...
    Vjyxc_l = Vjyxc_cam[:, :, :, 3:]
    Vjyxc_r = Vjyxc_cam[:, :, :, :3]
    Vjyxc_to_103 = np.concatenate([Vjyxc_l, Vjyxc_r], axis=0)

    print(Vjyxc_to_103.shape)
